    ...
    >
      ...
      {payload.map((p, i) => (
        <div key={i} style={{ color: p.color }}>
          {p.name}:{" "}
          <strong>
            {typeof p.value === "number" && p.name === "동의율"
              ? p.value.toFixed(1) + "%"
              : p.value}
          </strong>
        </div>
        ...
        {activeTab === "overview" && (
          <div>
            ...
            >
              ...
              <KpiCard
                label="상담 동의율"
                value={`${currentMonth.동의율.toFixed(1)}%`}
                sub={`${rateDelta > 0 ? "+" : ""}${rateDelta}% vs 전월`}
                trend={rateDelta >= 0 ? "up" : "down"}
                color="#0EA5E9"
              />
              <KpiCard
                label="이번달 매출"
                value={`${(CURRENT / 10000).toFixed(0)}만`}
                sub="목표 대비 79.7%"
                trend="up"
                color="#64748B"
              />
                  ...
                  <tbody>
                    {recentConsults.map((r, i) => (
                      <tr key={i} style={{ borderBottom: "1px solid #F8FAFC" }}>
                        ...
                        >
                          {r.proposed.toLocaleString()}원
                        </td>
                        <td
                          style={{
                            padding: "10px 12px",
                            fontWeight: 600,
                            color: r.agreed > 0 ? "#059669" : "#94A3B8",
                            whiteSpace: "nowrap",
                          }}
                        >
                    ...
                    dot={(props) => {
                      ...
                      return (
                        <circle
                          ...
                          cy={cy}
                          r={4}
                          fill={payload.동의율 >= 72 ? "#10B981" : "#EF4444"}
                          stroke="#fff"
                          strokeWidth={2}
                        />
                      );
              ...
              {[
                {
                  ...
                  value:
                    (
                      monthlyConsultData.reduce((a, b) => a + b.동의율, 0) / 12
                    ).toFixed(1) + "%",
                  color: "#2563EB",
                },
                {
                  label: "최고 동의율",
                  value:
                    Math.max(...monthlyConsultData.map((d) => d.동의율)).toFixed(1) +
                    "%",
                  color: "#10B981",
                },
                {
                  label: "최저 동의율",
                  value:
                    Math.min(...monthlyConsultData.map((d) => d.동의율)).toFixed(1) +
                    "%",
                  color: "#EF4444",
                },
                {
                  label: "총 상담 건수",
                  value:
                    monthlyConsultData.reduce((a, b) => a + b.상담수, 0) + "건",
                  color: "#64748B",
                },
              ].map((s, i) => (
                <div
            ...
            >
              ...
              >
                {[
                  ...
                  <div
                    key={i}
                    style={{ background: "#F8FAFC", borderRadius: 10, padding: "14px 16px" }}
                  >
                    <div
                      style={{
                        fontSize: 13,
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              >
                ...
                >
                  ...
                  <span style={{ fontSize: 16, color: "#94A3B8" }}>만원</span>
                </div>
              </div>
